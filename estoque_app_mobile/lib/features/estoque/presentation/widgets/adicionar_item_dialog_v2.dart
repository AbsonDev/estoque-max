...
class AdicionarItemDialogV2 extends StatefulWidget {
  final int? despensaId;

  const AdicionarItemDialogV2({
    super.key, 
    this.despensaId,
  });

  @override
  State<AdicionarItemDialogV2> createState() => _AdicionarItemDialogV2State();
}

class _AdicionarItemDialogV2State extends State<AdicionarItemDialogV2> {
  ...
  void initState() {
    super.initState();
    // Carregar despensas ao abrir o modal
                    ...
                    child: Form(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          // Resumo
                          if (_nomeProdutoDigitado != null && _nomeProdutoDigitado!.isNotEmpty)
                            _buildResumo(),
                        ],
                      ),
                    ),
  ...
  Widget _buildDespensaField() {
    return BlocSelector<EstoqueBloc, EstoqueState, EstoqueState>(
      selector: (state) {
        ...
        // Para outros estados, mantém o estado atual
        return state;
      },
      builder: (context, despensaState) {
        // Listener para definir despensa inicial quando carregadas
        if (despensaState is DespensasLoaded) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (_despensaSelecionada == null && despensaState.despensas.isNotEmpty) {
              setState(() {
                if (widget.despensaId != null) {
                  _despensaSelecionada = despensaState.despensas.firstWhere(
                    (d) => d.id == widget.despensaId,
                    orElse: () => despensaState.despensas.first,
                  );
                } else {
                  _despensaSelecionada = despensaState.despensas.first;
                }
              });
            }
          });
        ...
        if (despensaState is DespensasLoading) {
          return Container(
        ...
        if (despensaState is DespensasError) {
          return Container(
            padding: const EdgeInsets.all(16),
            decoration: BoxDecoration(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Erro ao carregar despensas: ${despensaState.message}',
        ...
        if (despensaState is DespensasLoaded) {
          return DespensaSelector(
            despensas: despensaState.despensas,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildResumoItem('Despensa', _despensaSelecionada?.nome ?? 'Não selecionada'),
          ...
          if (_quantidadeMinimaController.text.isNotEmpty)
            _buildResumoItem('Quantidade Mínima', _quantidadeMinimaController.text),
          if (_dataValidade != null)
            _buildResumoItem('Validade', DateFormat('dd/MM/yyyy').format(_dataValidade!)),
        ],
      ),
    );
  }
